...
export function SignatureDishes() {
  ...
  const { t } = useLanguage();
  const dishes = dishImages.map((image, i) => ({
    ...image,
    ...t.home.signatureDishes.dishes[i],
  }));
  ...
  return (
    <section className="bg-charcoal py-20 md:py-0">
      <div className="mx-auto max-w-[1440px] px-6 pt-0 md:hidden">
        <Eyebrow className="mb-8 text-warm-white/60">{t.home.signatureDishes.eyebrow}</Eyebrow>
        <h2 className="font-display text-3xl text-warm-white">
          {t.home.signatureDishes.title}
        </h2>
      </div>
      ...
      >
        <div className="hidden h-full flex-col justify-center px-12 md:absolute md:left-12 md:top-1/2 md:z-10 md:flex md:-translate-y-1/2">
          <Reveal>
            <Eyebrow className="mb-6 text-warm-white/60">{t.home.signatureDishes.eyebrow}</Eyebrow>
